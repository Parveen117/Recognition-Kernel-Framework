...
If an observable determines a signed or complex measure $\nu_f$ that is absolutely continuous with respect to $\mu$, the measure derivative is the Radon--Nikodym derivative
\begin{equation}
\frac{d\nu_f}{d\mu} \quad \text{whenever } \nu_f \ll \mu.
\end{equation}
For an ordinary function $f$, the notation $df/d\mu$ is used only after such a measure or Stieltjes interpretation has been declared.
...
When the chosen clock becomes flat, collapses distinctions, or loses resolution, the normalized derivative need not exist: the denominator no longer resolves the transition. This is not proof that nothing happens and not proof that the native transition vanishes; it is a loss of distinguishability in that chosen clock. An integrated seam, another lawful clock, or retained path memory may still detect the change.
...
\end{equation}
then the quotient defining $D_gf$ is unresolved on $U$ unless an additional limiting or quotient structure is declared. Flatness of $g$ records zero distinguishability under the selected measure; it does not by itself force internal variation of $f$ to vanish.
...
+ \sum_{k=1}^{n-1} \omega(O_n \circ \cdots \circ O_{k+1}, O_k \circ \cdots \circ O_1)
\end{multline}
\end{theorem}
\begin{proof}
Induct on the word length using twisted additivity. At each step the new defect is the cocycle evaluated on the new left factor and the previously composed prefix. The cocycle identity makes the resulting accumulated defect independent of parenthesization.
\end{proof}
...
\omega'(O_2,O_1)=\omega(O_2,O_1)+\alpha(O_2\circ O_1)-\alpha(O_2)-\alpha(O_1),
\]
then the two cocycle lifts are isomorphic under the re-presentation
\[
\Phi_\alpha(a,O)=(a+\alpha(O),O).
\]
Thus completed path recognition is invariant up to coboundary gauge.
\end{theorem}
\begin{proof}
Substitution into the two lifted composition laws gives identical memory coordinates. The inverse uses $-\alpha$.
\end{proof}
...
\end{theorem}
\begin{proof}
For a composable triple, the two bracketings have the same projected operator. Their memory coordinates differ exactly by
\[
\omega(O_3,O_2\circ O_1)+\omega(O_2,O_1)-\omega(O_3\circ O_2,O_1)-\omega(O_3,O_2),
\]
which vanishes exactly when the cocycle identity holds.
\end{proof}
...
\end{theorem}
\begin{proof}
With $\widetilde{\tr}(a,O)=a-\tr_\omega(O)$,
\[
\begin{aligned}
\widetilde{\tr}((a,O_2)\star(b,O_1))
&=a+b+\omega(O_2,O_1)-\tr_\omega(O_2\circ O_1)\\
&=a+b-\tr_\omega(O_2)-\tr_\omega(O_1),
\end{aligned}
\]
by twisted additivity, which is the required sum.
\end{proof}
...
\end{theorem}
\begin{proof}
The corrected completed trace is additive on $\star$, and $\exp(a+b)=\exp(a)\exp(b)$; therefore $\chi_{\tr}(X\star Y)=\chi_{\tr}(X)\chi_{\tr}(Y)$ and the identity maps to $1$.
\end{proof}
...
\begin{verbatim}
SOURCE_SHA256 ccc376bd423bb7266f9d66c2fb163596d35373f92790666767d60a01d115ffc1
VERIFIED_MAIN_SHA256_PREAPPENDIX 229d91e91d79e1271f8e31aebfb4b6ca5403daf0918660107acd53b9a086150d
...
\begin{enumerate}
\item \textbf{Holonomy Factorization}: \texttt{CERTIFIED\_ALGEBRAIC}.
\item \textbf{Gauge-Invariance}: \texttt{CERTIFIED\_ALGEBRAIC}.
\item \textbf{unnamed theorem 3}: \texttt{CERTIFIED\_ALGEBRAIC}.
\item \textbf{unnamed theorem 4}: \texttt{CERTIFIED\_ALGEBRAIC}.
\item \textbf{unnamed theorem 5}: \texttt{CERTIFIED\_ALGEBRAIC}.
\item \textbf{unnamed theorem 6}: \texttt{CONDITIONAL\_ON\_DECLARED\_REPRESENTATION}.
\item \textbf{unnamed theorem 7}: \texttt{INCOMPLETE\_OR\_OVERBROAD\_IN\_SOURCE}.
\item \textbf{unnamed theorem 8}: \texttt{INCOMPLETE\_OR\_OVERBROAD\_IN\_SOURCE}.
\item \textbf{FTC as Representation Theorem}: \texttt{CONDITIONAL\_ON\_DECLARED\_REPRESENTATION}.
\item \textbf{Foundational Closure}: \texttt{INCOMPLETE\_OR\_OVERBROAD\_IN\_SOURCE}.
\item \textbf{Complete Universality}: \texttt{INCOMPLETE\_OR\_OVERBROAD\_IN\_SOURCE}.
