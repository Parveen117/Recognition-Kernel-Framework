# Terminology and Filing Alignment

## Controlled public hierarchy
...
- `S_d`: domain or evidence state;
- `R_d`: recognition operator, predicate, invariant, or admissibility rule;
- `E_d`: evolution, transformation, theorem step, measurement, or update;
- `tau_d`: clock, index, dependency order, or execution sequence;
- `K_d`: kernel evaluator or recognition-gap computation.

## Mathematical proof-transport mapping

| RNKE term | Current mathematical cartridge |
|---|---|
...
Certified files retain historical paths, namespaces, schemas, and package IDs. In particular, `rh_framework` remains inside the imported cartridge because its source blob is pinned by the archived certificate. Historical identity is provenance, not the current public umbrella name.

## Filing-alignment notice
